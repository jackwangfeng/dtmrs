...
除了 env 里那个静态令牌，管理台还能按服务签发令牌，随时作废，并看到
**最后使用时间 / 调用次数 / 最近来源 IP** —— 判断「这个令牌还活着吗、
能不能安全作废」靠的就是这三个。

> HTTP 无连接、gRPC 的连接也不与令牌绑定，所以没有「当前连着几个客户端」
> 这种东西，别去找。

| 环境变量 | 作用 | 不配会怎样 |
|---|---|---|
| `DTMRS_TOKEN_KEY` | 加密令牌明文，让管理台能**事后复制** | 退化成「只显示一次」，丢了只能重发 |

```bash
DTMRS_TOKEN_KEY=$(openssl rand -hex 16) dtmrs
```

**为什么要加密而不是直接存明文**：「只显示一次」防的其实不是能进管理台的人 ——
那个人本来就能生成新令牌，看旧的并没有额外授权。它真正防的只有
**数据库层面的泄漏**（备份、只读副本、导出的 dump、共用同一个库的 DBA），
而 TC 存储和业务库共用是很常见的部署。

所以这里两个都要：**密文进库、密钥进配置文件**。dump 出去的库单独没用
（实测：换个 `DTMRS_TOKEN_KEY` 打开同一个库，复制明文返回 410），
而管理台随时能复制。

⚠ 密钥**刻意不从 `DTMRS_ADMIN_PASSWORD` 派生** —— 那样改管理台密码就会让
所有令牌的明文解不开，是个很难排查的耦合。

⚠ 换了 `DTMRS_TOKEN_KEY` **不影响令牌本身能不能用**（校验走哈希），
只影响「还能不能复制出明文」。

⚠ 令牌管理接口（`/api/admin/tokens*`）**只认会话 cookie，不认 Bearer**。
允许用 token 去增删 token 会形成提权链：一个泄漏的业务令牌可以自己再签发
一批，作废原来那个也没用。
...
**多套环境共用一个 Redis**（比如 staging 和 prod），给每套配一个 key 前缀，
它们就彼此完全看不见 —— 调度扫描也按前缀隔开，不会互相抢事务：

```bash
DTMRS_DB='redis://127.0.0.1:6379/0?key_prefix=staging:'  dtmrs
```

不配就是 `dtmrs:`。前缀只收 `A-Z a-z 0-9 : _ . -`、不能为空，写错会在启动时报错。
同一套环境的多个 TC 实例要用**同一个**前缀，否则它们等于各自一套，接不了彼此的活。
