...
⚠ **Redis 的默认是 `appendonly no`，AOF 根本是关的**，只有 RDB 快照；默认的
`save 3600 1 ...` 意味着低峰期最坏能丢**一小时**的事务状态。
（`appendfsync everysec` 在 AOF 关闭时完全不起作用，别看到它就以为有保障。）

对协调器来说丢的是**事务状态**：可能出现「业务侧已经扣了款，但 TC 这边没有这笔事务」
的悬挂。SQL 后端不会（提交即落盘）。

最要命的是**分支已经跑了一部分时丢掉记录** —— 那些分支永远不会被补偿或确认。
而这恰好是最近写入的部分，正是崩溃时最容易丢的。

**最低要求 `appendonly yes` + `appendfsync everysec`。**
资金类强一致场景干脆别用 Redis 做 TC 存储 —— 即使 `appendfsync always`，
Redis 主从复制是异步的，failover 一样会丢已确认的写入，AOF 救不了。
详见 [部署文档](docs/deployment.md#redis-的三条语义差异)。
...
| `grpc` | gRPC 服务端接口 + 调 `grpc://` `grpcs://` 分支 |
