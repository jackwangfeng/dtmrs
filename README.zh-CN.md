...
| 模式 | dtmrs | DTM v1.19 | |
|---|---|---|---|
| saga，2 个步骤 | **14112 笔/秒** | 9582 | dtmrs 快 47% |
| msg，1 个正向步骤 | **12371 笔/秒** | 10058 | dtmrs 快 23% |

saga 这一栏原来是**输 22%**（7630 vs 9339）。差距的来源就是上面那次抢占往返：
DTM 在 submit 请求里同步推完、不入队，而我们每笔都要先抢一次。
把抢占折进建事务那条写入之后，saga 从落后 22% 变成领先 47%。

（msg 这条路还没做内联 —— 它的 `submit_prepared` 要把事务体一起返回才能直接推，
是笔额外的活，而这条路本来就赢着。这是已知的剩余空间。）
